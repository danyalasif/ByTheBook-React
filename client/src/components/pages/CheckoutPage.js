...
import { ListGroup } from '../../../node_modules/react-bootstrap';
import { DecreaseQty, IncreaseQty, ClearCart } from 'react-redux-shopping-cart';
import { Grid, Row, Col, Well, Image } from '../../../node_modules/react-bootstrap';
        ...
        return (
            ...
            {!loading && books.length < 1 ? (
                <Message>Nothing In Cart :( </Message>
                    ...
                    <Item.Extra>
                    {cart.items.filter(x =>  x.id === book.book_id._id).map(y =>  
                    
                    
                    y.qty <= 5 ? <IncreaseQty
                    itemId={book.book_id._id}
                    styles={generalStyles + incrementStyles}
                    />       : <Button
                    disabled
                    type="number"
                    styles={generalStyles + incrementStyles + `background-color: red`}
                    >No Quantity </Button>)}
                                 
                    
                    <DecreaseQty
                    itemId={book.book_id._id}
                    styles={generalStyles + decrementStyles}                   
                    />
                    </Item.Extra>
                    <Item.Extra>
                    ...
                    placeholder="0"
                    name="quantity"
                    value={cart.items.filter(x =>  x.id === book.book_id._id).map(y =>  y.qty)}
                    ...
                    actionPosition="left"
                    placeholder="0"
                    value={cart.items.filter(x =>  x.id === book.book_id._id).map(y => y.subTotal)}
                    />
...

{
    ...
    {!loading && books.length < 1 ? (
        <Message>Nothing In Cart :( </Message>
            ...
            actionPosition="left"
            placeholder="0"
            value={cart.items.map(x => x.id === book.book_id._id ? x.qty : 0)}
            style={{appearance: "none"}}
            />
